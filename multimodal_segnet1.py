...
class Up(nn.Module):
    def __init__(self, in_c, skip_c, out_c):
        ...
        self.conv = ConvBlock(in_c=skip_c + skip_c, out_c=out_c)
...
class SharedDistinctFusion(nn.Module):
    ...
    def forward(self, f2d, fceus):
        ...
        return torch.cat([shared, d2d, dceus], dim=1)
...
class MultiModalSegNet(nn.Module):
    def __init__(self, in_ch=1, out_ch=1):
        ...
        self.down1_2d = Down(64, 128)

        self.enc_ceus = CEUSEncoder()

        #self.fusion = SharedDistinctFusion(in_c=64)
        self.fusion = SharedDistinctFusion(c2d=64, cceus=64)
        self.up1 = Up(in_c=192, skip_c=64, out_c=64)
        self.outc = nn.Conv2d(64, out_ch, 1)

    def forward(self, x2d, xceus):
        f2d = self.down1_2d(self.inc2d(x2d))
        fceus = self.enc_ceus(xceus)
        fceus_up = F.interpolate(fceus, size=f2d.shape[2:], mode='bilinear', align_corners=False)
        fcat = self.fusion(f2d, fceus_up)
        x = self.up1(fcat, self.inc2d(x2d))
        return self.outc(x)
